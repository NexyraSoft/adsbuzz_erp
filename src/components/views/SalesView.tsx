        ...
        <div className="lg:col-span-7 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm shadow-slate-100 dark:shadow-none min-h-[500px] flex flex-col justify-between">
          <form onSubmit={handleCheckoutSubmit} id="checkout-form" className="space-y-6">
            ...
            {currentStep === 1 && (
              ...
              >
                <div>
                  <h3 className="text-base font-bold text-slate-950 dark:text-white font-sans">Who is purchasing?</h3>
                  <p className="text-xs text-slate-400 mt-0.5">Select an existing customer or onboard a new customer directly within this sales entry.</p>
                </div>
            ...
            {currentStep === 2 && (
              ...
              >
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2.5">Publisher Platform</label>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {(['Facebook', 'TikTok', 'Google', 'Snapchat'] as PlatformType[]).map((p) => {
                        const isSelected = platform === p;
                        let activeStyles = 'border-[#1F5E98] bg-blue-50/30 text-[#1F5E98] ring-2 ring-[#1F5E98]/10';
                        if (isSelected) {
                          if (p === 'Facebook') {
                            activeStyles = 'border-[#1877F2] bg-blue-50/30 text-[#1877F2] ring-2 ring-[#1877F2]/15';
                          } else if (p === 'TikTok') {
                            activeStyles = 'border-[#FE2C55] bg-rose-50/30 text-[#FE2C55] ring-2 ring-[#FE2C55]/15';
                          } else if (p === 'Google') {
                            activeStyles = 'border-[#4285F4] bg-sky-50/30 text-[#4285F4] ring-2 ring-[#4285F4]/15';
                          } else if (p === 'Snapchat') {
                            activeStyles = 'border-[#d4b200] bg-yellow-50/40 text-[#b29400] ring-2 ring-[#FFFC00]/30';
                          }
                        }
                        return (
                          <button
                            ...
                            className={`p-3 rounded-xl border text-xs font-bold transition-all text-center flex flex-col items-center justify-center gap-2 cursor-pointer ${
                              isSelected 
                                ? activeStyles 
                  ...
                  {activeAccount && (() => {
                    ...
                    return (
                      <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-800 space-y-3 text-xs bg-slate-50/50 dark:bg-slate-900/40">
                        <div className="flex justify-between">
                          <span className="text-slate-400">BM Hub:</span>
                          <span className="font-bold text-slate-700 dark:text-slate-300">{activeAccount.bmName || "AdsBuzz Partner"}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-slate-400">Assigned Card:</span>
                          <span className="font-mono font-medium text-slate-700 dark:text-slate-300">{activeAccount.billingCard || "None Linked"}</span>
                        </div>

                        <div className="pt-3 border-t border-slate-200 dark:border-slate-800 space-y-2">
                          <div className="flex justify-between items-center">
                            <span className="font-bold text-slate-900 dark:text-white">Ad Account History:</span>
                            <span className="text-[10px] bg-[#F68B2D]/15 text-[#F68B2D] px-2 py-0.5 rounded-full font-black">
                              {matchingInvoices.length} {matchingInvoices.length === 1 ? 'top-up' : 'top-ups'}
                            </span>
                          </div>
                          
                          <div className="grid grid-cols-2 gap-2 text-center">
                            <div className="bg-white dark:bg-slate-900 p-2 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
                              <p className="text-[9px] text-slate-400 font-extrabold uppercase tracking-wider">Total USD Top-up</p>
                              <p className="text-sm font-black text-[#F68B2D] mt-0.5">${totalUSD.toLocaleString()}</p>
                            </div>
                            <div className="bg-white dark:bg-slate-900 p-2 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
                              <p className="text-[9px] text-slate-400 font-extrabold uppercase tracking-wider">Total BDT Spent</p>
                              <p className="text-sm font-black text-[#1F5E98] mt-0.5">৳{totalBDT.toLocaleString()}</p>
                            </div>
                          </div>
                        </div>
                      </div>
            ...
            {currentStep === 4 && (
              ...
              >
                ...
                <div className="space-y-4 border border-slate-200 dark:border-slate-800 rounded-xl p-4 bg-slate-50/50 dark:bg-slate-900/30">
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    <div>
                      <p className="text-slate-400">Customer</p>
                      <p className="font-bold text-slate-800 dark:text-slate-200">{activeCustomer?.name || 'N/A'}</p>
                      <p className="text-[10px] text-slate-500">{activeCustomer?.companyName}</p>
                    </div>
                    <div>
                      <p className="text-slate-400">Publisher Platform</p>
                      <p className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                        <span className={`inline-block w-2.5 h-2.5 rounded-full ${
                          platform === 'Facebook' ? 'bg-[#1877F2]' :
                          platform === 'TikTok' ? 'bg-[#FE2C55]' :
                          platform === 'Google' ? 'bg-[#4285F4]' : 'bg-[#FFFC00]'
                        }`} />
                        {platform}
                      </p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs pt-3 border-t border-slate-200/50 dark:border-slate-800/50">
                    <div>
                      <p className="text-slate-400">Target Ad Account</p>
                      <p className="font-bold text-slate-800 dark:text-slate-200">{activeAccount?.adAccountName || 'N/A'}</p>
                      <p className="text-[10px] font-mono text-slate-500">ID: {activeAccount?.adAccountId}</p>
                    </div>
                    <div>
                      <p className="text-slate-400">Billing BM Hub</p>
                      <p className="font-bold text-slate-800 dark:text-slate-200">{activeAccount?.bmName || 'AdsBuzz Partner'}</p>
                    </div>
                  </div>

                  <div className="grid grid-cols-3 gap-2 text-xs pt-3 border-t border-slate-200/50 dark:border-slate-800/50 text-center">
                    <div className="bg-white dark:bg-slate-900 p-2.5 rounded-lg border border-slate-200/60 dark:border-slate-800/60">
                      <p className="text-[10px] text-slate-400 uppercase tracking-wider font-semibold">USD Top-up</p>
                      <p className="text-xs font-bold text-[#F68B2D] mt-0.5">${topupAmountUSD}</p>
                    </div>
                    <div className="bg-white dark:bg-slate-900 p-2.5 rounded-lg border border-slate-200/60 dark:border-slate-800/60">
                      <p className="text-[10px] text-slate-400 uppercase tracking-wider font-semibold">Dollar Rate</p>
                      <p className="text-xs font-bold text-slate-700 dark:text-slate-300 mt-0.5">৳{dollarRate}</p>
                    </div>
                    <div className="bg-white dark:bg-slate-900 p-2.5 rounded-lg border border-slate-200/60 dark:border-slate-800/60">
                      <p className="text-[10px] text-slate-400 uppercase tracking-wider font-semibold">Total BDT</p>
                      <p className="text-xs font-bold text-[#1F5E98] mt-0.5">৳{totalBDT.toLocaleString()}</p>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-4 text-xs pt-3 border-t border-slate-200/50 dark:border-slate-800/50">
                    <div>
                      <p className="text-slate-400">BDT Amount Paid</p>
                      <p className="font-bold text-emerald-600">৳{paidBDT.toLocaleString()}</p>
                    </div>
                    <div>
                      <p className="text-slate-400">Remaining Due</p>
                      <p className={`font-bold ${dueBDT > 0 ? 'text-red-500' : 'text-emerald-600'}`}>
                        ৳{dueBDT.toLocaleString()}
                      </p>
                    </div>
                  </div>

                  <div className="grid grid-cols-3 gap-4 text-xs pt-3 border-t border-slate-200/50 dark:border-slate-800/50">
                    <div>
                      <p className="text-slate-400">Payment Channel</p>
                      <p className="font-bold text-slate-800 dark:text-slate-200">{paymentMethod}</p>
                    </div>
                    <div>
                      <p className="text-slate-400">Execution Status</p>
                      <span className={`inline-block text-[10px] px-2 py-0.5 rounded font-bold mt-0.5 ${
                        topupStatus === 'Successfull' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950/20' :
                        topupStatus === 'Pending' ? 'bg-amber-50 text-amber-600 dark:bg-amber-950/20' : 'bg-red-50 text-red-600 dark:bg-red-950/20'
                      }`}>
                    ...
                    <div>
                      ...
                      <span className={`inline-block text-[10px] px-2 py-0.5 rounded font-bold mt-0.5 ${
                        approvalStatus === 'Approved' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950/20' :
                        approvalStatus === 'Pending' ? 'bg-amber-50 text-amber-600 dark:bg-amber-950/20' : 'bg-red-50 text-red-600 dark:bg-red-950/20'
                      }`}>
                        {approvalStatus}
                      </span>
                    </div>
                  ...
                  {noteText && (
                    <div className="pt-3 border-t border-slate-200/50 dark:border-slate-800/50 text-xs">
                      <p className="text-slate-400">Auditor Notes</p>
                      <p className="text-slate-600 dark:text-slate-400 mt-0.5 italic">&ldquo;{noteText}&rdquo;</p>
                    </div>
                  )}
                </div>
